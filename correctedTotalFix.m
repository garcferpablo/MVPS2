function [figure2, table2, sigComparisons2] = correctedTotalFix(S)
% Igual que fig 4 pero para fijaciones corregidas con el Método de Molitor 
% et al. (2014). 
% Memory-based fixation counts were calculated by subtracting the number of 
% fixations for correct foil trials (i.e., correct rejections) from all other
% trial types (Molitor et al., 2014). Thus, positive scores denote more 
% fixations than correct foil trials and negative scores denote fewer fixations
% than correct foil trials.
%
% Entradas:
...
for i = 1:nS
      for j = 1:nR
        ...
        foil = tbl.Subject==subjs(i) & tbl.RespTag==4;

        mR = mean(tbl.RecFix(sel),'omitnan');
        fR = mean(tbl.RecFix(foil),'omitnan');
        meanRec(i,j) = mR - fR;

        m1 = mean(tbl.CodFix1(sel),'omitnan'); f1 = mean(tbl.RecFix(foil),'omitnan');
        meanCod1(i,j) = m1 - f1;
        m2 = mean(tbl.CodFix2(sel),'omitnan'); f2 = mean(tbl.RecFix(foil),'omitnan');
        meanCod2(i,j) = m2 - f2;
        m3 = mean(tbl.CodFix3(sel),'omitnan'); f3 = mean(tbl.RecFix(foil),'omitnan');
        meanCod3(i,j) = m3 - f3;
      end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ylabel('Media de Fijaciones Totales Corregidas','FontWeight','bold');
    legend(hb2, responseLabels2, 'Location','NorthWest');
    %title('Solo hits, LCR y LFA','FontSize',14);
    ylim([-0.5, max(Y2(:)+E2(:))*1.2]);
    hold off
